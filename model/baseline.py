...
class CustomImageDataset(torch.utils.data.Dataset):
    ...
    def create_fname_list(self):
        ...
        self.fname_list = list(filter(lambda x: os.stat('data/resized/label/' + x)[6] > 3000, raw))
        self.fname_list = list(filter(lambda x: x.find("jpg") != -1, self.fname_list))
...
def main():
    ...
    train_dataset = CustomImageDataset('data/resized')
    ...
    if args.save_model:
        torch.save(model.state_dict(), "siamese_networklabel.pt")
